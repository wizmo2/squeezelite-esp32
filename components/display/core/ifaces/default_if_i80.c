...
static bool I80DefaultWriteBytes( spi_device_handle_t SPIHandle, int WriteMode, const uint8_t* Data, size_t DataLength );
static bool I8ODefaultWriteCommand( struct GDS_Device* Device, uint8_t Command );
static bool I80DefaultWriteData( struct GDS_Device* Device, const uint8_t* Data, size_t DataLength );

bool GDS_I80Init( int SPI, int DC ) {
	SPIHost = SPI;
	DCPin = DC;
    return true;
}

bool GDS_I80AttachDevice( struct GDS_Device* Device, int Width, int Height, int CSPin, int RSTPin, int BackLightPin, int Speed, int Mode ) {
...
static bool I80DefaultWriteCommand( struct GDS_Device* Device, uint8_t Command ) {
    ...
    return I80DefaultWriteBytes( Device->SPIHandle, GDS_SPI_Command_Mode, &CommandByte, 1 );
}
...
static bool I80DefaultWriteData( struct GDS_Device* Device, const uint8_t* Data, size_t DataLength ) {
    ...
    return I80DefaultWriteBytes( Device->SPIHandle, GDS_SPI_Data_Mode, Data, DataLength );
}